...
class Product extends Model
{
    ...
    public function vendorPrice()
    {
        ...
        $price = $this->price;
        // !!!!!!!
        // if a special commission has been applied to vendor set commission to price
        // !!!!!!!
        if ($this->user_id != 0) {
            $vendor = User::find($this->user_id);
            //$price = $this->price + $gs->fixed_commission + ($this->price/100) * $gs->percentage_commission ;
            $price = $this->price + ($this->price / 100) * $vendor->vendor_commission;
    ...
    public function showPreviousPrice()
    {
        ...
        if ($this->user_id != 0) {
            ...
            $price = $this->previous_price + ($this->previous_price / 100) * $vendor->vendor_commission;
        }
    ...
    public function releatedProducts()
    {
        return $this->hasMany('App\Models\Product', 'category_id', 'category_id')->where('status', 1)->where('id', '!=', $this->id)->orderBy('id', 'DESC')->limit(8);
    }
