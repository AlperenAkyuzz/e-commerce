...
class ProductController extends Controller
{
    ...
    public function createPhysical()
    {
        ...
        $cats = Category::all();
        $sign = Currency::where('is_default', '=', 1)->first();
        return view('vendor.product.create.physical', compact('cats', 'sign', 'shippers'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        if ($prods < $package->allowed_products || $package->allowed_products == 0) {
           ...
           /* if ($file = $request->file('photo')) {
            ...
            //--- Redirect Section
            $msg = 'Ürün başarıyla eklendi.<a href="' . route('vendor-prod-index') . '">Ürün Listesini Görüntüle</a>';
            return response()->json($msg);
            //--- Redirect Section Ends
        } else {
            //--- Redirect Section
    ...
    public function edit($id)
    {
        $cats = Category::all();
        $data = Product::findOrFail($id);
        // Check if user is the product author
    ...
    public function update(Request $request, $id)
    {
        ...
        //--- Redirect Section
        $msg = 'Ürün Başarıyla Güncellendi.<a href="' . route('vendor-prod-index') . '">Ürün Listesini Görüntüle</a>';
        return response()->json($msg);
        //--- Redirect Section Ends
    }
