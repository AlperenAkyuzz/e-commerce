...
class DashboardController extends Controller
{
    ...
    {
        $activation_data = "";
        if (file_exists(public_path().'/system/license.txt')){
            $license = file_get_contents(public_path().'/system/license.txt');
            if ($license != ""){
    ...
    public function activation_submit(Request $request)
    {
        ...
        if($chk['status'] != "success")
        {
            ...
            $fpbt = fopen(public_path().'/system/license.txt', 'w');
    ...
    public function movescript(){
        ...
        $files = public_path();
        $bkupname = 'System-Backup-'.date('Y-m-d').'.zip';
