...
class GeneralSettingController extends Controller
{
    ...
    {
        $path = public_path().'assets/front/';

        $jsContent = file_get_contents($path . 'js/gs-custom.js');

        $cssContent = file_get_contents($path . 'css/gs-custom.css');

        return view('admin.generalsetting.customjs', compact('jsContent', 'cssContent'));
    }
    ...
    public function updateCustomJS(Request $request) {
        $jsContent = $request->input('js_code');
        $cssContent = $request->input('css_code');

        $path = 'assets/front/';

        $jsStorage = file_put_contents($path . 'js/gs-custom.js', $jsContent);
        $cssStorage = file_put_contents($path . 'css/gs-custom.css', $cssContent);

        if($jsStorage != false && $cssStorage != false) {
            return 'Veriler güncellendi';
        } else {
            return 'Lütfen system/app/storage/public/ dizinini kontrol ediniz';
        }
    }
