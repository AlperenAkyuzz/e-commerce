...
class CatalogController extends Controller
{
    ...
    {
        //return 'Henüz yapım aşamasında';
        return view('theme::pages.categories');
    }
    ...
    public function category(Request $request, $slug = null, $slug1 = null, $slug2 = null)
    {
        ...
        if(!empty($vendors)) {
            $prods = $prods->where(function ($query) use ($vendors) {
                $query->whereIn('user_id', $vendors);
                //var_dump($vendors);
            });
        }

        $prods = $prods->where('status', 1)->with('brand', 'user')->get();
        $brands = $prods->pluck('brand')->unique();
        $pUsers = $prods->pluck('user')->unique();
        ...
        if ($request->ajax()) {
            ...
            return view('theme::includes.product.filtered-products', $data);
            //return view('includes.product.filtered-products', $data);
        }
        //return 'Henüz yapım aşamasında';
        return view('theme::pages.category.category', $data);
    }
