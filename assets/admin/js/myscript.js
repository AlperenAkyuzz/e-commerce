                    ...
                    <div class="row">
                      ...
                      <div class="col-10">
                        <div class="price-container">
                          ...
                          <input type="text" class="input-field price-input" id="${data[i].attribute.input_name}${data[i].options[j].id}_price" data-name="${data[i].attribute.input_name}_price[]" placeholder="0.00 (Ekstra Fiyat)" value="">
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="col-lg-7 ${priceClass}">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <div class="price-container">
                            <span class="price-curr">${curr.sign}</span>
                            <input type="text" class="input-field price-input" id="${data[i].attribute.input_name}${data[i].options[j].id}_price" data-name="${data[i].attribute.input_name}_price[]" placeholder="0.00 ({{ __('Additional Price') }})" value="">
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="col-lg-7 ${priceClass}">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <div class="price-container">
                            <span class="price-curr">${curr.sign}</span>
                            <input type="text" id="${data[i].attribute.input_name}${data[i].options[j].id}_price" class="input-field price-input" data-name="${data[i].attribute.input_name}_price[]" placeholder="0.00 (Ekstra Fiyat)" value="">
                          </div>
                        </div>
                      </div>
                    </div>
...
// ADD OPERATION
        ...
        $(document).on('click', '#add-data', function () {
            ...
            $('#modal1').find('.modal-title').html('YENİ ' + $('#headerdata').val() + ' Ekle');
...
// DELIVERY OPERATION
        ...
        $(document).on('click', '.delivery', function () {
            ...
            $('#modal1').find('.modal-title').html('Teslimat Durumu');
            $('#modal1 .modal-content .modal-body').html('').load($(this).attr('data-href'), function (response, status, xhr) {
                if (status == "success") {
                    if (admin_loader == 1) {
                        $('.submit-loader').hide();
...
// ADD / EDIT FORM SUBMIT FOR DATA TABLE
        ...
        $(document).on('submit', '#parionformdata', function (e) {
            e.preventDefault();
            if (admin_loader == 1) {
                $('.submit-loader').show();
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ((data.errors)) {
                        ...
                        $('#parionformdata input , #parionformdata select , #parionformdata textarea').eq(1).focus();
                    } else {
                        table.ajax.reload();
                        $('.alert-success').show();
                        $('.alert-success p').html(data);
...
// NORMAL FORM
    ...
    $(document).on('submit', '#parionform', function (e) {
        e.preventDefault();
        if (admin_loader == 1) {
            $('.gocover').show();
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                //console.log(data);
                if ((data.errors)) {
                    parionform.parent().find('.alert-success').hide();
                    parionform.parent().find('.alert-danger').show();
                    parionform.parent().find('.alert-danger ul').html('');
                    for (var error in data.errors) {
                        $('.alert-danger ul').append('<li>' + data.errors[error] + '</li>')
                    }
                    parionform.find('input , select , textarea').eq(1).focus();
                } else {
                    parionform.parent().find('.alert-danger').hide();
                    parionform.parent().find('.alert-success').show();
                    parionform.parent().find('.alert-success p').html(data);
                    parionform.find('input , select , textarea').eq(1).focus();
                }
                if (admin_loader == 1) {
                    $('.gocover').hide();
